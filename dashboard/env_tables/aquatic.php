<?php 
    ...
    $joinQuery = "FROM `{$table}` AS `t1` JOIN `new_meta_studies` AS `t2` ON (`t1`.`CMIM_ID` = `t2`.`CMIM_ID`) JOIN `source` AS `t3` ON (`t1`.`Source_ID` = `t3`.`Source_ID`) JOIN `states` AS `t4` ON (`t1`.`state_id` = `t4`.`state_id`) JOIN `main_environments` AS `t5` ON (`t1`.`menv_id` = `t5`.`menv_id`) JOIN `environments` AS `t6` ON (`t1`.`env_id` = `t6`.`env_id`) ";

    $where = "`t5`.`main_environment`= 'Aquatic' AND `t6`.`env_name`!= 'Marine'";
