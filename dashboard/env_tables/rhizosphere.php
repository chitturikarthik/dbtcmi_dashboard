<?php 
    ...
    $columns = array(
        ...
        array( 'db' => '`t3`.`env_name`', 'dt' => 3, 'field' => 'env_name' ),
        array( 'db' => '`t4`.`state_name`', 'dt' => 4, 'field' => 'state_name' )
    );

    $joinQuery = "FROM `{$table}` AS `t1` JOIN `new_meta_studies` AS `t2` ON (`t1`.`CMIM_ID` = `t2`.`CMIM_ID`) JOIN `environments` AS `t3` ON (`t1`.`env_id` = `t3`.`env_id`) JOIN `states` AS `t4` ON (`t1`.`state_id` = `t4`.`state_id`) JOIN `source` AS `t5` ON (`t1`.`Source_ID` = `t5`.`Source_ID`) ";

    $where = "`t5`.`source_name` = 'Rhizosphere' OR `t5`.`source_name` = 'Root'";
