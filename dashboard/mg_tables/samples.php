<?php 
    ...
    $columns = array(
        ...
        array( 'db' => '`t1`.`Location`', 'dt' => 6, 'field' => 'Location' )
    );

    $joinQuery = "FROM `{$table}` AS `t1` JOIN `new_meta_studies` AS `t2` ON (`t1`.`CMIM_ID` = `t2`.`CMIM_ID`) JOIN `source` AS `t3` ON (`t1`.`Source_ID` = `t3`.`Source_ID`) JOIN `states` AS `t4` ON (`t1`.`state_id` = `t4`.`state_id`) ";
